...
6. **Ratchet 학습 — 양방향 4 방향 (헌장 § 2.F5)**

   사용자에게 *네 방향 모두* 묻는다. 각 방향은 `AGENTS.md` 의 대응 섹션에 한 줄로 누적 (단방향, 제거 금지). **없으면 "없다" 가 정답** (거짓 반성은 노이즈).

   | 질문 | AGENTS.md 섹션 | 방향 |
   |------|---------------|------|
   | "다시는 일어나면 안 되는 실수가 있었나요?" | `## 절대 하지 말 것` | 음의 학습 |
   | "이번에 잘 작동해 다음에도 쓰고 싶은 패턴이 있나요?" | `## 검증된 패턴` | 양의 학습 |
   | "Operator 가 가르친 도메인 단어·규칙이 있나요?" | `## 운영자가 가르친 것` | Operator → Engineer |
   | "Operator 가 처음 알게 된 소프트웨어 가능성이 있나요?" | `## 운영자에게 보여준 가능성` | Engineer → Operator |

   - 추가 전 사용자에게 정확한 문구를 보여주고 승인받는다 (구체적이어야 함 — "잘 하자" ❌)
   - `.harness/failure-log` 에 기록된 실패가 있다면 음의 학습의 좋은 후보 — 패턴을 묶어 한 줄로 압축
   - **양방향 비대칭 주의**: 음의 학습만 쌓이고 양의·도메인·가능성이 *영원히 0* 이면 FDE 가 아니라 컨설팅 신호 (헌장 § 2.F5)
